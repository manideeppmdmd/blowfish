...
class blowfishE:
      ...
      def f(self,plaintxt):
          ...
          for i in range(0,8,2):
              col=int(self.hextobin(plaintxt[i:i+2]),2)
              a=int(i/2)
              list[a]=self.s[a][col]
          ans=self.addBin(list[0],list[1])
          ans=self.xor(ans,list[2])
          ans=self.addBin(ans,list[3])
          return(ans)
     
      def keygernation(self,key):
          j=0
          for i in range(len(self.p)):
              #print(key[j:j+8])
              self.p[i]=self.xor(self.p[i],key[j:j+8])
             # print("subkey"+str(i+1)+" : "+str(self.p[i]))
              j=(j+8)%(len(key))
      
      def round(self,i,plaintxt):
              left=plaintxt[:8]
              right=plaintxt[8:]
              left =self.xor(left, self.p[i])
              if len(left)<8:
                  left="0" +left[:]
              fOut=self.f(left)
              ...
              if len(right)<8:
                  right ="0" +right[:]
              #print("round "+ str(i)+" : "+ right + left)
              return right + left
      
      def encrypt(self,plaintxt):
          for i in range(16):
              plaintxt=self.round(i,plaintxt)
              #print(plaintxt)
          #plaintxt=self.round(11,"1106c1ab8b574312")
          right=plaintxt[:8]
          left=plaintxt[8:]
...
def main():
    ...
    b.keygernation(key)
    plaintxt="68696968656c6c6f"
    ciphertxt =b.encrypt(plaintxt)
    print(ciphertxt)'''

    userinput = input('Enter name of the file to be encrypted : ')
    key=input("Enter key 32bits :  ")
    t0= time.perf_counter()
    with open(userinput,'r') as rf:
         i=0
         for line in rf:
             plaintxt = line[:16]
             b.keygernation(key)
             ciphertxt =b.encrypt(plaintxt)
             i+=1
             with open('EnText.enc','a') as file:
                 file.seek(0)
                 file.write(ciphertxt)
                 if i <31:
                     file.write("\n")
    t1= time.perf_counter()
    print("File has been encryted and stored as: Entext.enc")
