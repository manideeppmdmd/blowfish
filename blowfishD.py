...
def main():
  ...
  b= blowfishD()
  userinput = input('Enter name of the file to be decrypted : ')
  ...
  with open(userinput,'r') as rf:
         ...
         for line in rf:
             plaintxt = line[:16]
             b.keygernation(key)
             ciphertxt =b.decrypt(plaintxt)
             i+=1
             #print(ciphertxt)
             with open('DeText.txt','a') as file:
                 file.seek(0)
                 file.write(ciphertxt)
                 if i<31:
                     file.write("\n")
  t1= time.perf_counter()
  print("File has been encryted and stored as: DeText.txt")
